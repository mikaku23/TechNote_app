...
@section('title', 'Data Terhapus Instalasi Software')
@section('css')
            ...
            <div class="card mt-3">
                <div class="card-header d-flex justify-content-between">
                    <div class="header-title">
                        <h4 class="card-title">Data Terhapus Instalasi Software</h4>
                    </div>
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            <tr class="text-center deleted-row">
                                ...
                                <td>{{ $soft->user->nama ?? '-' }}</td>
                                <td>{{ $soft->software->nama ?? '-' }}</td>
                                <td>{{ $soft->status }}</td>
                                <td>{{ $soft->tgl_hapus ? $soft->tgl_hapus->format('d F Y') : '-' }}</td>
                                <td>
                                    <form action="{{ route('penginstalan.pulihkan', $soft->id) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('PATCH')
